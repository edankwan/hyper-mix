...
void main() {
    ...
    if(center.z > 0.001) {
        ...
        vec3 sum = vec3( 0.0 );
        float sumC = 0.0;
        vec4 color = texture2D( tDiffuse, vec2( vUv.x, vUv.y - 4.0 * uOffset ) );
        sum += color.xyz * 0.051;
        sumC += fract(color.w + 0.5) * 0.051;

        color = texture2D( tDiffuse, vec2( vUv.x, vUv.y - 3.0 * uOffset ) );
        sum += color.xyz * 0.0918;
        sumC += fract(color.w + 0.5) * 0.0918;

        color = texture2D( tDiffuse, vec2( vUv.x, vUv.y - 2.0 * uOffset ) );
        sum += color.xyz * 0.12245;
        sumC += fract(color.w + 0.5) * 0.12245;

        color = texture2D( tDiffuse, vec2( vUv.x, vUv.y - 1.0 * uOffset ) );
        ...
        color = texture2D( tDiffuse, vec2( vUv.x, vUv.y + 4.0 * uOffset ) );
        sum += color.xyz * 0.051;
        sumC += fract(color.w + 0.5) * 0.051;
